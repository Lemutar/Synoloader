...
var QuickConnect = function(timeout_relay, timeout_internal, protocol, port) {
    ...
    this.check_internal_ips = function(internal_ips, callback) {
        ...
        internal_ips.forEach(function(ip) {
            var get_server = Request(
                protocol + ip + ":" + port + "/webapi/query.cgi", "api=SYNO.API.Info&version=1&method=query&query=api=SYNO.API.Info&version=1&method=query&query=SYNO.API.Auth,SYNO.DownloadStation.Task",
                timeout_internal,
                function(response) {
                    if (response.status == 200) {
